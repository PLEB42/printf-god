#include "ft_printf.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <limits.h>
#include <stdint.h>

static __attribute__((unused)) char *s_hidden = "hi low\0don't print me lol\0";
static __attribute__((unused)) char *s2 = "hello";
static __attribute__((unused)) int i_pos_1 = 878023;
static __attribute__((unused)) char ch_pos_1 = 100, ch_neg_1 = -87;
static __attribute__((unused)) short sh_pos_1 = 3047, sh_neg_1 = -8875;
static __attribute__((unused)) long l_pos_1 = 22337203685477, l_neg_1 = -22337203685477;
static __attribute__((unused)) long long ll_pos_1 = 22337203685477, ll_neg_1 = -22337203685477;
static __attribute__((unused)) unsigned char uch_pos_1 = 100;
static __attribute__((unused)) unsigned short ush_pos_1 = 3047;
static __attribute__((unused)) unsigned int ui_pos_1 = 878023;
static __attribute__((unused)) unsigned long ul_pos_1 = 22337203685477;
static __attribute__((unused)) unsigned long long ull_pos_1 = 22337203685477;
static __attribute__((unused)) long lmax = 9223372036854775807;
static __attribute__((unused)) long lmin = -9223372036854775807;
static __attribute__((unused)) long long llmax = 9223372036854775807;
static __attribute__((unused)) long long llmin = -9223372036854775807ll;
static __attribute__((unused)) unsigned long ulmax = 9223372036854775807;
static __attribute__((unused)) unsigned long long ullmax = 9223372036854775807;
static __attribute__((unused)) int ncm_p = 5;
static __attribute__((unused)) char a01;
static __attribute__((unused)) unsigned char a02;
static __attribute__((unused)) short a03;
static __attribute__((unused)) unsigned short a04;
static __attribute__((unused)) int a05;
static __attribute__((unused)) unsigned int a06;
static __attribute__((unused)) long a07;
static __attribute__((unused)) unsigned long a08;
static __attribute__((unused)) long long a09;
static __attribute__((unused)) unsigned long long a10;
static __attribute__((unused)) char *a11;
static __attribute__((unused)) void *a12;
static __attribute__((unused)) int i;
static __attribute__((unused)) long l;
static __attribute__((unused)) char *str = "abc";
static __attribute__((unused)) int t = 42;
static __attribute__((unused)) void test_moulitest_06(void) {}

int main(int argc, char **argv)
{
	if (argc < 2) return 1;
	int test_num = atoi(argv[1]);
	int ret = 0;
	if (test_num == 1)
		ret = ft_printf("%i", 42);
	else if (test_num == 2)
		ret = ft_printf("%i", -42);
	else if (test_num == 3)
		ret = ft_printf("before %i after", 42);
	else if (test_num == 4)
		ret = ft_printf("%i%i%i%i%i", 1, -2, 3, -4, 5);
	else if (test_num == 5)
		ret = ft_printf("a%ib%ic%id", 1, -2, 3);
	else if (test_num == 6)
		ret = ft_printf("%+i", 42);
	else if (test_num == 7)
		ret = ft_printf("%+i", -42);
	else if (test_num == 8)
		ret = ft_printf("% i", 9999);
	else if (test_num == 9)
		ret = ft_printf("% i", -9999);
	else if (test_num == 10)
		ret = ft_printf("%.4i", 42);
	else if (test_num == 11)
		ret = ft_printf("%15.4i", 42);
	else if (test_num == 12)
		ret = ft_printf("%4.15i", 42);
	else if (test_num == 13)
		ret = ft_printf("%.i, %.0i", 0, 0);
	else if (test_num == 14)
		ret = ft_printf("%-i", 42);
	else if (test_num == 15)
		ret = ft_printf("this %i number", 17);
	else if (test_num == 16)
		ret = ft_printf("this %i number", -267);
	else if (test_num == 17)
		ret = ft_printf("this %i number", 0);
	else if (test_num == 18)
		ret = ft_printf("%i", 3);
	else if (test_num == 19)
		ret = ft_printf("%i", -1);
	else if (test_num == 20)
		ret = ft_printf("%i", 0);
	else if (test_num == 21)
		ret = ft_printf("%i",  2147483647);
	else if (test_num == 22)
		ret = ft_printf("%i", (int)(-2147483678));
	else if (test_num == 23)
		ret = ft_printf("%0+8.5i", 34);
	else if (test_num == 24)
		ret = ft_printf("%0+10.5i", -216);
	else if (test_num == 25)
		ret = ft_printf("%0+8.5i", 0);
	else if (test_num == 26)
		ret = ft_printf("%0+8.3i", 8375);
	else if (test_num == 27)
		ret = ft_printf("%0+8.3i", -8473);
	else if (test_num == 28)
		ret = ft_printf("%0+3.7i", 3267);
	else if (test_num == 29)
		ret = ft_printf("%0+3.7i", -2375);
	else if (test_num == 30)
		ret = ft_printf("%0+3.3i", 6983);
	else if (test_num == 31)
		ret = ft_printf("%0+3.3i", -8462);
	else if (test_num == 32)
		ret = ft_printf("%+i", 5);
	else if (test_num == 33)
		ret = ft_printf("%+i", -7);
	else if (test_num == 34)
		ret = ft_printf("%+i", 2147483647);
	else if (test_num == 35)
		ret = ft_printf("%+5i", 35);
	else if (test_num == 36)
		ret = ft_printf("%+7i", 0);
	else if (test_num == 37)
		ret = ft_printf("%+24i", 2147483647);
	else if (test_num == 38)
		ret = ft_printf("%+.7i", 234);
	else if (test_num == 39)
		ret = ft_printf("%+.7i", -446);
	else if (test_num == 40)
		ret = ft_printf("%+.3i", 3723);
	else if (test_num == 41)
		ret = ft_printf("%+05i", 432);
	else if (test_num == 42)
		ret = ft_printf("%+04i", 0);
	else if (test_num == 43)
		ret = ft_printf("%+8.5i", 34);
	else if (test_num == 44)
		ret = ft_printf("%+10.5i", -216);
	else if (test_num == 45)
		ret = ft_printf("%+8.5i", 0);
	else if (test_num == 46)
		ret = ft_printf("%+8.3i", 8375);
	else if (test_num == 47)
		ret = ft_printf("%+8.3i", -8473);
	else if (test_num == 48)
		ret = ft_printf("%+3.7i", 3267);
	else if (test_num == 49)
		ret = ft_printf("%+3.7i", -2375);
	else if (test_num == 50)
		ret = ft_printf("%+3.3i", 6983);
	else if (test_num == 51)
		ret = ft_printf("%+3.3i", -8462);
	else if (test_num == 52)
		ret = ft_printf("%+-8.5i", 34);
	else if (test_num == 53)
		ret = ft_printf("%+-10.5i", -216);
	else if (test_num == 54)
		ret = ft_printf("%+-8.5i", 0);
	else if (test_num == 55)
		ret = ft_printf("%+-8.3i", 8375);
	else if (test_num == 56)
		ret = ft_printf("%+-8.3i", -8473);
	else if (test_num == 57)
		ret = ft_printf("%+-3.7i", 3267);
	else if (test_num == 58)
		ret = ft_printf("%+-3.7i", -2375);
	else if (test_num == 59)
		ret = ft_printf("%+-3.3i", 6983);
	else if (test_num == 60)
		ret = ft_printf("%+-3.3i", -8462);
	else if (test_num == 61)
		ret = ft_printf("%hhi", (char)45);
	else if (test_num == 62)
		ret = ft_printf("%hhi", (char)-45);
	else if (test_num == 63)
		ret = ft_printf("%hi", (short)385);
	else if (test_num == 64)
		ret = ft_printf("%hi", (short)-385);
	else if (test_num == 65)
		ret = ft_printf("%hhi", ch_pos_1);
	else if (test_num == 66)
		ret = ft_printf("%hhi", ch_neg_1);
	else if (test_num == 67)
		ret = ft_printf("%hi", sh_pos_1);
	else if (test_num == 68)
		ret = ft_printf("%hi", sh_neg_1);
	else if (test_num == 69)
		ret = ft_printf("%hi", (short)32767);
	else if (test_num == 70)
		ret = ft_printf("%hi", (short)(-32768));
	else if (test_num == 71)
		ret = ft_printf("%hhi", (char)127);
	else if (test_num == 72)
		ret = ft_printf("%hhi", (char)(-128));
	else if (test_num == 73)
		ret = ft_printf("%037li", 22337203685477l);
	else if (test_num == 74)
		ret = ft_printf("%.37li", -22337203685477l);
	else if (test_num == 75)
		ret = ft_printf("%37lli", 522337203685470ll);
	else if (test_num == 76)
		ret = ft_printf("%-37lli", -522337203685470ll);
	else if (test_num == 77)
		ret = ft_printf("%+li", 22337203685477l);
	else if (test_num == 78)
		ret = ft_printf("[%i] [%i]", i_pos_1, i_pos_1);
	else if (test_num == 79)
		ret = ft_printf("[%i] [%hi]", i_pos_1, sh_pos_1);
	else if (test_num == 80)
		ret = ft_printf("[%i] [%hhi]", i_pos_1, ch_pos_1);
	else if (test_num == 81)
		ret = ft_printf("this % i number", 17);
	else if (test_num == 82)
		ret = ft_printf("this % i number", -267);
	else if (test_num == 83)
		ret = ft_printf("this % i number", 0);
	else if (test_num == 84)
		ret = ft_printf("% i", 3);
	else if (test_num == 85)
		ret = ft_printf("% i", -1);
	else if (test_num == 86)
		ret = ft_printf("% i", 0);
	else if (test_num == 87)
		ret = ft_printf("% i", 2147483647);
	else if (test_num == 88)
		ret = ft_printf("% i", (int)(-2147483678));
	else if (test_num == 89)
		ret = ft_printf("% 7i", 33);
	else if (test_num == 90)
		ret = ft_printf("% 7i", -14);
	else if (test_num == 91)
		ret = ft_printf("% 3i", 0);
	else if (test_num == 92)
		ret = ft_printf("% 5i", 52625);
	else if (test_num == 93)
		ret = ft_printf("% 5i", -2562);
	else if (test_num == 94)
		ret = ft_printf("% 4i", 94827);
	else if (test_num == 95)
		ret = ft_printf("% 4i", -2464);
	else if (test_num == 96)
		ret = ft_printf("% -7i", 33);
	else if (test_num == 97)
		ret = ft_printf("% -7i", -14);
	else if (test_num == 98)
		ret = ft_printf("% -3i", 0);
	else if (test_num == 99)
		ret = ft_printf("% -5i", 52625);
	else if (test_num == 100)
		ret = ft_printf("% -5i", -2562);
	else if (test_num == 101)
		ret = ft_printf("% -4i", 94827);
	else if (test_num == 102)
		ret = ft_printf("% -4i", -2464);
	else if (test_num == 103)
		ret = ft_printf("% .5i", 2);
	else if (test_num == 104)
		ret = ft_printf("% .6i", -3);
	else if (test_num == 105)
		ret = ft_printf("% .3i", 0);
	else if (test_num == 106)
		ret = ft_printf("% .4i", 5263);
	else if (test_num == 107)
		ret = ft_printf("% .4i", -2372);
	else if (test_num == 108)
		ret = ft_printf("% .3i", 13862);
	else if (test_num == 109)
		ret = ft_printf("% .3i",-23646);
	else if (test_num == 110)
		ret = ft_printf("% 05i", 43);
	else if (test_num == 111)
		ret = ft_printf("% 07i", -54);
	else if (test_num == 112)
		ret = ft_printf("% 03i", 0);
	else if (test_num == 113)
		ret = ft_printf("% 03i", 634);
	else if (test_num == 114)
		ret = ft_printf("% 04i", -532);
	else if (test_num == 115)
		ret = ft_printf("% 04i", -4825);
	else if (test_num == 116)
		ret = ft_printf("% 8.5i", 34);
	else if (test_num == 117)
		ret = ft_printf("% 10.5i", -216);
	else if (test_num == 118)
		ret = ft_printf("% 8.5i", 0);
	else if (test_num == 119)
		ret = ft_printf("% 8.3i", 8375);
	else if (test_num == 120)
		ret = ft_printf("% 8.3i", -8473);
	else if (test_num == 121)
		ret = ft_printf("% 3.7i", 3267);
	else if (test_num == 122)
		ret = ft_printf("% 3.7i", -2375);
	else if (test_num == 123)
		ret = ft_printf("% 3.3i", 6983);
	else if (test_num == 124)
		ret = ft_printf("% 3.3i", -8462);
	else if (test_num == 125)
		ret = ft_printf("% -8.5i", 34);
	else if (test_num == 126)
		ret = ft_printf("% -10.5i", -216);
	else if (test_num == 127)
		ret = ft_printf("% -8.5i", 0);
	else if (test_num == 128)
		ret = ft_printf("% -8.3i", 8375);
	else if (test_num == 129)
		ret = ft_printf("% -8.3i", -8473);
	else if (test_num == 130)
		ret = ft_printf("% -3.7i", 3267);
	else if (test_num == 131)
		ret = ft_printf("% -3.7i", -2375);
	else if (test_num == 132)
		ret = ft_printf("% -3.3i", 6983);
	else if (test_num == 133)
		ret = ft_printf("% -3.3i", -8462);
	else if (test_num == 134)
		ret = ft_printf("% 08.5i", 34);
	else if (test_num == 135)
		ret = ft_printf("% 010.5i", -216);
	else if (test_num == 136)
		ret = ft_printf("% 08.5i", 0);
	else if (test_num == 137)
		ret = ft_printf("% 08.3i", 8375);
	else if (test_num == 138)
		ret = ft_printf("% 08.3i", -8473);
	else if (test_num == 139)
		ret = ft_printf("% 03.7i", 3267);
	else if (test_num == 140)
		ret = ft_printf("% 03.7i", -2375);
	else if (test_num == 141)
		ret = ft_printf("% 03.3i", 6983);
	else if (test_num == 142)
		ret = ft_printf("% 03.3i", -8462);
	else if (test_num == 143)
		ret = ft_printf("% hhi", (char)45);
	else if (test_num == 144)
		ret = ft_printf("% hhi", (char)-45);
	else if (test_num == 145)
		ret = ft_printf("% hi", (short)385);
	else if (test_num == 146)
		ret = ft_printf("% hi", (short)-385);
	else if (test_num == 147)
		ret = ft_printf("% li", (long)32);
	else if (test_num == 148)
		ret = ft_printf("% li", (long)-32);
	else if (test_num == 149)
		ret = ft_printf("% lli", (long long)43);
	else if (test_num == 150)
		ret = ft_printf("% lli", (long long)-43);
	else if (test_num == 151)
		ret = ft_printf("% hhi", ch_pos_1);
	else if (test_num == 152)
		ret = ft_printf("% hhi", ch_neg_1);
	else if (test_num == 153)
		ret = ft_printf("% hi", sh_pos_1);
	else if (test_num == 154)
		ret = ft_printf("% hi", sh_neg_1);
	else if (test_num == 155)
		ret = ft_printf("% li", l_pos_1);
	else if (test_num == 156)
		ret = ft_printf("% li", l_neg_1);
	else if (test_num == 157)
		ret = ft_printf("% lli", ll_pos_1);
	else if (test_num == 158)
		ret = ft_printf("% lli", ll_neg_1);
	else if (test_num == 159)
		ret = ft_printf("% li", 22337203685477l);
	else if (test_num == 160)
		ret = ft_printf("% li", -22337203685477l);
	else if (test_num == 161)
		ret = ft_printf("% lli", 522337203685470ll);
	else if (test_num == 162)
		ret = ft_printf("% lli", -522337203685470ll);
	else if (test_num == 163)
		ret = ft_printf("% lli", 9223372036854775807ll);
	else if (test_num == 164)
		ret = ft_printf("% lli", llmin);
	else if (test_num == 165)
		ret = ft_printf("% li", 9223372036854775807l);
	else if (test_num == 166)
		ret = ft_printf("% li", lmin);
	else if (test_num == 167)
		ret = ft_printf("% hi", (short)32767);
	else if (test_num == 168)
		ret = ft_printf("% hi", (short)(-32768));
	else if (test_num == 169)
		ret = ft_printf("% hhi", (char)(127));
	else if (test_num == 170)
		ret = ft_printf("% hhi", (char)(-128));
	else if (test_num == 171)
		ret = ft_printf("%+.0i", 0);
	else if (test_num == 172)
		ret = ft_printf("%+.i", 0);
	else if (test_num == 173)
		ret = ft_printf("%+5.0i", 0);
	else if (test_num == 174)
		ret = ft_printf("%+5.i", 0);
	else if (test_num == 175)
		ret = ft_printf("%+-5.0i", 0);
	else if (test_num == 176)
		ret = ft_printf("%+-5.i", 0);
	else if (test_num == 177)
		ret = ft_printf("%7i", 33);
	else if (test_num == 178)
		ret = ft_printf("%7i", -14);
	else if (test_num == 179)
		ret = ft_printf("%3i", 0);
	else if (test_num == 180)
		ret = ft_printf("%5i", 52625);
	else if (test_num == 181)
		ret = ft_printf("%5i", -2562);
	else if (test_num == 182)
		ret = ft_printf("%4i", 94827);
	else if (test_num == 183)
		ret = ft_printf("%4i", -2464);
	else if (test_num == 184)
		ret = ft_printf("%-7i", 33);
	else if (test_num == 185)
		ret = ft_printf("%-7i", -14);
	else if (test_num == 186)
		ret = ft_printf("%-3i", 0);
	else if (test_num == 187)
		ret = ft_printf("%-5i", 52625);
	else if (test_num == 188)
		ret = ft_printf("%-5i", -2562);
	else if (test_num == 189)
		ret = ft_printf("%-4i", 94827);
	else if (test_num == 190)
		ret = ft_printf("%-4i", -2464);
	else if (test_num == 191)
		ret = ft_printf("%.5i", 2);
	else if (test_num == 192)
		ret = ft_printf("%.6i", -3);
	else if (test_num == 193)
		ret = ft_printf("%.3i", 0);
	else if (test_num == 194)
		ret = ft_printf("%.4i", 5263);
	else if (test_num == 195)
		ret = ft_printf("%.4i", -2372);
	else if (test_num == 196)
		ret = ft_printf("%.3i", 13862);
	else if (test_num == 197)
		ret = ft_printf("%.3i",-23646);
	else if (test_num == 198)
		ret = ft_printf("%05i", 43);
	else if (test_num == 199)
		ret = ft_printf("%07i", -54);
	else if (test_num == 200)
		ret = ft_printf("%03i", 0);
	printf("\n[RET: %d]", ret);
	return 0;
}
