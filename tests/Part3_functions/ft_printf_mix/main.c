...
int main(int argc, char **argv)
{
	...
	if (test_num == 1)
		...
		ret = ft_printf("Hallo heimur");
	else if (test_num == 23)
		ret = ft_printf("Mix: %c %s %p %d %i %u %x %X %%", 'A', "test", (void *)0xdeadbeef, 42, -42, 42, 42, 42);
	else if (test_num == 24)
		ret = ft_printf("Mix: %10c %-10s %p %010d %i %u %x %X %%", 'B', "hello", (void *)0x1234, 123, -123, 123, 123, 123);
	else if (test_num == 25)
		ret = ft_printf("Mix: %c %s %p %d %i %u %x %X %%", 0, "", (void *)0, 0, 0, 0, 0, 0);
	else if (test_num == 26)
		ret = ft_printf("%s %d %p %x %X %u %i %c %%", "str", 10, (void *)0xabcdef, 255, 255, 4294967295u, -1, 'z');
	else if (test_num == 27)
		ret = ft_printf("%-10s %10d %p %x %X %u %i %c %%", "left", 12345, (void *)0x1, 0, 0, 0, 0, ' ');
	else if (test_num == 28)
		ret = ft_printf("%.5s %.5d %p %x %X %u %i %c %%", "longstring", 42, (void *)&a01, 0xabc, 0xABC, 100, 100, '!');
	else if (test_num == 29)
		ret = ft_printf("%s %d %p %x %X %u %i %c %%", "limits", INT_MIN, (void *)0, 0, 0, 0, 0, '7');
	else if (test_num == 30)
		ret = ft_printf("Multiple: %d %d %d %d %d %s %s %c %c", 1, 2, 3, 4, 5, "one", "two", 'a', 'b');
	else if (test_num == 31)
		ret = ft_printf("Hex: %#x %#X %p %d", 0x123, 0x123, (void *)0x123, 0x123);
	else if (test_num == 32)
		ret = ft_printf("Mixed again: %%%c%s%p%d%i%u%x%X%%", 'Z', "final", (void *)&t, 42, 42, 42, 42, 42);
	printf("\n[RET: %d]", ret);
	return 0;
}
